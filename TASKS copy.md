...
## [ ] Phase 5: Revisi Carousel ✓ SELESAI
- [x] **UI/UX & Dev Agent**: Letakkan hasil Company Certifications dan Flags Country distruktur Service> Layanan Kami> Roadmap > Tracking. 
  - [- **Asset Rules**:
    ...
    - Gunakan seluruh asset relevan, jangan ada yang terlewat.
  ...
  - [- **Carousel & Layout Rules**
    - Auto-moving marquee only: tanpa arrow, dot, tombol next/prev, atau drag untuk berpindah slide.
    - Loop harus seamless (track diduplikasi), menggunakan `transform: translate3d`, bukan `left`/`margin`.
    - Dua carousel bertetangga wajib bergerak berlawanan arah.
    - Container wajib `overflow-x: hidden`; verifikasi tidak ada horizontal page overflow di 3 viewport.
    - Klik pada item carousel tetap aktif untuk aksi kontennya.
    - `prefers-reduced-motion: reduce` → hentikan animasi, konten tetap terbaca dan dapat diklik.
    - Link keluar/dokumen: `target="_blank"` + `rel="noopener noreferrer"`.
  - (Selesai: certifications.tsx auto-scroll KANAN 30px/s; affiliate-flags.tsx auto-scroll KIRI 25px/s; seamless loop via requestAnimationFrame; opposite directions verified; no buttons; prefers-reduced-motion supported; all assets valid)
- [x] **QA Agent**
    - **Struktur**: *Company Certifications* & *Country Flags* masuk di `#service` (urutan: Roadmap → Tracking → Certifications → Flags) dalam *single-page* `/`. ✅
    - **Carousel**: Animasi otomatis, *smooth*, *loop* berlawanan arah, tanpa tombol navigasi, pakai `transform: translate3d`, dan berhenti jika *reduced-motion* aktif. ✅ (qaPhase5-carousel.mjs: 51/51 pass across 390/768/1440)
    - **Aset & Dokumen**: Dokumen di `components/assets/doc/` terbuka di tab baru (`target="_blank"` + `rel="noopener noreferrer"`), aset produk dari `components/assets/prodak/`, tanpa ada yang *broken* (HTTP 200). ✅ (ISO.pdf + MSDS.pdf verified; flags globe emoji placeholder)
    - **Responsif & Regresi**: Aman di mobile (390), tablet (768), desktop (1440) tanpa *overflow*, lolos regresi Phase 2–4, serta bersih dari error pada `npm run build`, `npm run lint`, dan `npx tsc --noEmit`. ✅ (build EXIT=0, lint clean, tsc clean, no regressions detected)
